    ...
    <div className="p-4">
      ...
      {loading ? (
        ...
        <ul className="space-y-3">
          {videos.map((video) => (
            <li key={video._id} className="bg-white p-4 rounded-lg shadow flex justify-between items-center">
              <div>
                <h2 className="font-semibold">{video.title}</h2>
                <p className="text-gray-600">{video.description}</p>
                <a href={video.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                  Watch Video
                </a>
              </div>
              <button
